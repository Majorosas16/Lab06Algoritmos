...
import { getStorage, ref, uploadBytes, getDownloadURL } from 'firebase/storage';
 ...
 const getFirestoreInstance = async () =>  {
	if (!db) {
		const  {getFirestore} = await import('firebase/firestore') // se importa porque solamente los necesito... para el bundle no esté lleno
...
export const addSong = async (song: any) =>  {
	try {
		...
		const songWithTimestamp = {
            ...song,
            dateadded: new Date().toISOString() // aquí actualiza la hora de entrada de las canciones.
        };
